...
def _tariff_table() -> str:
    return table(
        ["", "BASIC", "PRO"],
        [
            ["Цена", "999 ₽/мес", "2 999 ₽/мес"],
            ["3 месяца", "2 697 ₽", "8 097 ₽"],
            ["Менеджеры", "1", "10"],
            ["Лимит сделок", "20/мес", "Безлимит"],
            ["Кастом.статусы", "—", "✅"],
            ["Приём платежей", "—", "✅"],
            ["Статистика", "—", "✅"],
            ["Задачи", "—", "✅"],
...
def _pitch_tariff_html() -> str:
    return doc(
        heading(1, "🚀 Добро пожаловать в Leady!"),
        para(
            "Вы ведёте бизнес в Telegram: заказы, клиенты, команда — всё в чатах. "
            "Но клиенты теряются, сделки забываются."
        ),
        para("<b>Leady — CRM внутри Telegram.</b> Сделки, клиенты и задачи в одном месте, без лишних окон."),
        divider(),
        heading(2, "🔥 Что вы получаете"),
        "<p>• <b>Сделки</b> — заявка ⟶ переговоры ⟶ оплата</p>",
        "<p>• <b>Клиенты</b> — вся история в одном месте</p>",
        "<p>• <b>Задачи</b> — контроль сроков сотрудников</p>",
        "<p>• <b>Статистика</b> — выручка, конверсия, средний чек (PRO)</p>",
        "<p>Всё — в вашем Telegram-боте. Клиенты даже не узнают, что вы используете CRM.</p>",
        divider(),
        heading(2, "⚡ Как это работает"),
        "<p>1️⃣ Выбираете тариф</p>",
        "<p>2️⃣ Создаёте бота за 1 минуту</p>",
        "<p>3️⃣ Сотрудники заходят и работают</p>",
        "<p>4️⃣ Клиенты оставляют заявки — вы их не теряете</p>",
        "<p><i>Никаких веб-сайтов. Чистый Telegram.</i></p>",
        divider(),
        heading(2, "💳 Сравнение тарифов"),
        table(
            ["", "BASIC", "PRO"],
...
async def onboarding_bot_username(message: Message, state: FSMContext) -> None:
    ...
    if username in STOP_WORDS or username.rstrip("bot").rstrip("_") in STOP_WORDS:
        await message.answer("⚠️ Это имя зарезервировано. Придумайте другое.")
        return
